/* BaseDeConhecimento.java
 * Criado em 10/09/2015, atualizado em 02/03/2020
 * @author  Alex Souza
...
public class BaseDeConhecimento extends JFrame implements ActionListener {
    
    // Criando os Labels
	JLabel lb_Codigo, lb_Categoria, lb_Produto, lb_Nucleo, lb_Titulo, lb_Descricao,
	lb_Solucao, lb_Obs,	lb_Ativo, lb_Autor, lb_Data, lb_Link, lb_Caminho, lb_AlexSouza, lb_Pesquisar;
	
    // Criando os TextField
	JTextField tf_Codigo, tf_Titulo, tf_Data, tf_Link, tf_Caminho, tf_Pesquisar;
	
	JTextArea ta_Descricao, ta_Solucao, ta_Obs;
	
	// Criando ComboBox
	JComboBox cb_Nucleo;
	String[] Nucleo = { "Suporte","Infraestrutura", "Negócio", "Contratos", "Dados e Inteligência"};
	
	JComboBox cb_Categoria;
	String[] Categoria = { "Ficha de Servidores", "Erros", "Procedimentos", "Cadastros", "Problemas", "Alteração", "Configuração", "Verificação", "Habilitação", "Instalação", "Atualização", "Aquisição", "Backup", "Desempenho", "Solicitação"};
    
	JComboBox cb_Produto;
	String[] Produto = { "ERP - XXX ", "CRM - XXX", "BI", "Tableau", "PowerBI", "Sistema de Monitoramento", "SAP", "SQL Server", "Microsoft Windows", "Microsoft Windows Server", 
			             "Celular", "Impressora", "Rede", "Conta Celular", "Modem 3G", "Modem 4G", "Notebook", "Tablets", "Computador", "Fortgate",
			             "Navegadores", "InfraEstrutura", "Negócios", "Dados", "AI"};
	
	JComboBox cb_Autor;
	String[] Autor = { "Alex Souza", "Antonio Souza", "Alex Silva", "Usuário 2", "Usuário 3", "Usuário 4"};
    ...
    public BaseDeConhecimento (String titulo)
    {
        super("Z Solutions - Base de Conhecimento");
        iniciaComponentes();
    ...
    private void iniciaComponentes()
    {
    	...
    	scDesc.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
       	...
       	lb_Solucao     = new JLabel("Solução");
       	ta_Solucao     = new JTextArea();
        ...
        this.addWindowListener(new WindowAdapter()
        {
            public void windowClosing(java.awt.event.WindowEvent evt)
            {
            	...
            	if (sair == JOptionPane.YES_OPTION) {
            		BaseDeConhecimento.this.dispose();
            	} else if (sair == JOptionPane.NO_OPTION) {
            		//BaseDeConhecimento.tf_Pesquisar.requestFocus();
        ...
        // Solução
        lb_Solucao.setBounds(10,205,70,25);
        painelPrincipal.add(lb_Solucao);
        ta_Solucao.setBounds(80,205,420,200);
        painelPrincipal.add(ta_Solucao);
        ta_Solucao.setLineWrap(true); 
        ta_Solucao.setWrapStyleWord(true);
	...
	private void DadosDB()
	{
		try
		{
			...
			rs = stmt.executeQuery("SELECT * FROM TbBaseConhecimento ORDER BY Codigo;");
			rs.next();
            PreencheCampos();
		}
		catch (Exception e)
	...
	private void PreencheCampos() // Começa a pegar informações do BD.
	{
        try
	       { 
		      ...
		      ta_Solucao.setText(rs.getString("Solucao"));     
		      ...
		      //tf_Data.setText(formatador.format(rs.getString("Data")));
        ...
        if (evt.getSource() == bt_Proximo)
         ...
         if (evt.getSource() == bt_Adiciona)
            ...
            {               
               ...
               stmt.executeUpdate("INSERT INTO TbBaseConhecimento VALUES ('"+b+"','"+c+"','"+d+"','"+e+"','"+f+"','"+g+"','"+h+"','"+i+"','"+j+"',getdate(),'"+m+"','"+n+"')");
               rs = stmt.executeQuery("SELECT * FROM TbBaseConhecimento ORDER BY Codigo;");
         ...
         if (evt.getSource() == bt_PesquisarCodigo)      	 
             ...
             {  System.out.println("Pesq. Cód");
                ...
                rs = stmt.executeQuery("SELECT * FROM TbBaseConhecimento WHERE Codigo = " + p);
 	     ...
 	     if (evt.getSource() == bt_Pesquisar)      	 
 	         try
 	         {
 	            ...
 	            rs = stmt.executeQuery("SELECT * FROM TbBaseConhecimento WHERE Titulo    like '%"+p+"%'" 
 	          	+                "UNION SELECT * FROM TbBaseConhecimento WHERE Descricao like '%"+p+"%'"
 	          	+                "UNION SELECT * FROM TbBaseConhecimento WHERE Solução   like '%"+p+"%'");
 	            	...
 	            	JFrame w = new JFrame("Resultado da pesquisa");
  	                ...
  	                String content = "Resultado da Pesquisa\n"
	            		           + "-------------------------------\n"
  	                               ...
  	                               + "Solução: "   + rs.getString(7)  + "\n"
  	                             //+ "Obs: "       + rs.getString(8)  + "\n"
  	                               + "Ativo: "     + rs.getString(9)  + "\n"
  	                             //+ "Autor: "     + rs.getString(10) + "\n"
         ...
         if (evt.getSource() == bt_Remove)
         {
        	...
        	if (excluir == JOptionPane.YES_OPTION) {
                try
                {
                   ...
                   stmt.executeUpdate("DELETE FROM  TbBaseConhecimento WHERE Codigo = " + a);
                   rs = stmt.executeQuery("SELECT * FROM TbBaseConhecimento ORDER BY Codigo;");
                   rs.next();
                   PreencheCampos();
         ...
         try
         {
            ...
            stmt.executeUpdate("UPDATE TbBaseConhecimento "
            		         + "SET Categoria = '"+b+"'"
            		         + ",   Produto   = '"+c+"'"
            		         + ",   Nucleo    = '"+d+"'"
            		         + ",   Titulo    = '"+e+"'"
            ...
            rs = stmt.executeQuery("SELECT * FROM TbBaseConhecimento WHERE Codigo >= "+a+" ORDER BY Codigo;");
            rs.next();
    ...
    public static void main(String args[]){
        BaseDeConhecimento cli = new BaseDeConhecimento("Z Solutions - Base de Conhecimento - ITIL");
